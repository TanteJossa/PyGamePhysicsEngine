...
def create_arrow(begin=(100, 100), end=(200, 400), width=5, arrow_size=None, color=blue, text=None, textColor=None, textfont=fontfreesan, textposoffset=0):
    angle = atan2((end[1]-begin[1]), (end[0]-begin[0] + 0.0001))
    ...
    pointTriBeginX = begin[0] + (length - arrow_length) * cos(angle)
    pointTriBeginY = begin[1] + (length - arrow_length) * sin(angle)

    textpos = (begin[0] + (length / 2) * cos(angle) + 10 + textfont.size(text)[0] / 2, begin[1] + (length / 2) * sin(angle) - 5 - textfont.size(text)[1] / 2 + textposoffset)
    vertecies = [(begin[0] + rotatexoffset, begin[1] - rotateyoffset), 
                (pointTriBeginX + rotatexoffset, pointTriBeginY - rotateyoffset),
                (pointTriBeginX + 2 * rotatexoffset, pointTriBeginY - 2 * rotateyoffset),
                (end[0], end[1]),
                (pointTriBeginX - 2 * rotatexoffset, pointTriBeginY + 2 * rotateyoffset),
                (pointTriBeginX - rotatexoffset, pointTriBeginY + rotateyoffset),
...
#create Baumgartner
Baumgartner = physics_object(name='Baumgartner',startposy=39045, startposx=20000, startvelx=-100, massa=80, opp=2, drag=0.7, useParachute=True, parachuteOpp=42.6, parachuteDrag=1.75, parachuteDeployTime=5, parachuteDeployHeight=3000)
